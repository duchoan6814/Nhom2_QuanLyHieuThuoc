...
public class pnlQuanLyPhieuNhap extends JPanel {
	private JTextField txtNhpSHa;
	private TableModel hoaDonModel;
	private JTable tblHoaDon;
	private ArrayList<PhieuNhap> hoaDonHienThi;
	...
	public pnlQuanLyPhieuNhap() {
		...
		String[] colHeader = { "STT", "Mã phiếu nhập", "Ngày lập", "Nhân Viên", "Tổng tiền" };
		...
		JButton btnTtCHa = new JButton("Tất cả phiếu nhập");
	...
	public void addAllHoaDonToTable() {
		hoaDonHienThi = new DAOPhieuNhap().getAllPhieuNhap();
		if (hoaDonHienThi == null) {
			...
			for (PhieuNhap hoaDon : hoaDonHienThi) {
				((DefaultTableModel) hoaDonModel)
						.addRow(new Object[] { hoaDonModel.getRowCount(), hoaDon.getMaPhieuNhap(), hoaDon.getNgayNhap(),
								hoaDon.getNhanVien().getMaNhanVien(), hoaDon.tinhTongGiaNhap() });
			}
	...
	public void addHoaDonToTableTuNgayToiNgay(String startDate, String endDate) {
		((DefaultTableModel) hoaDonModel).setRowCount(0);
		hoaDonHienThi = new DAOPhieuNhap().getPhieuNhapTuNgayDenNgay(startDate, endDate);
		if (hoaDonHienThi == null) {
			JOptionPane.showMessageDialog(null, "Có lỗi sảy ra trong quá trình lấy hóa đơn!");
		} else {
			lblSoDong.setText("Số dòng: " + Integer.toString(hoaDonHienThi.size()));
	...
	public void addHoaDonToTableTheoNhanVien(String ho, String ten) {
		((DefaultTableModel) hoaDonModel).setRowCount(0);
		hoaDonHienThi = new DAOPhieuNhap().getListPhieuNhapTheoNhanVien(ho, ten);
		if (hoaDonHienThi == null) {
			JOptionPane.showMessageDialog(null, "Có lỗi sảy ra trong quá trình lấy hóa đơn!");
		} else {
			lblSoDong.setText("Số dòng: " + Integer.toString(hoaDonHienThi.size()));
			for (PhieuNhap hoaDon : hoaDonHienThi) {
				((DefaultTableModel) hoaDonModel).addRow(new Object[] { hoaDonModel.getRowCount(), hoaDon.getMaPhieuNhap(), hoaDon.getNgayNhap(),
						hoaDon.getNhanVien().getMaNhanVien(), hoaDon.tinhTongGiaNhap() });
			}
		}
	}
